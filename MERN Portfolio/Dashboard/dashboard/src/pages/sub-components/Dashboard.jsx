...
import { clearAllSoftwareAppErrors, deleteSoftwareApplication, getsoftwareApplications, resetSoftwareApplicationSlice } from "@/slices/softwareApplicationSlice";
...
import { clearAllSkillSliceErrors } from "@/slices/skillSlice";
import { clearAllProjectErrors } from "@/slices/projectSlice";
import { clearAllTimelineErrors } from "@/slices/timelineSlice";
import { clearAllexperienceErrors, deleteexperience } from "@/slices/exprienceSlice";
...
export const Dashboard=()=>{
    ...
    const {skills,
        ...
        error: skillError,
        message: skillMessage,}=useSelector((state)=>state.skill);

    const {softwareApplications,
        loading: appLoading,
        error: appError,
        message: appMessage,}=useSelector((state)=>state.softwareApplications);

    const {timeline,
        loading: timelineLoading,
        error: timelineError,
        message: timelineMessage,}=useSelector((state)=>state.timeline);
    
      const {experience,
        loading: experienceLoading,
        error: experienceError,
        message: experienceMessage,}=useSelector((state)=>state.experience);
                                            ...
                                            experience.map((element) => {
                                              // console.log(element)
                                                ...
                                                <TableRow className="bg-accent" key={element._id}>
                                                    ...
                                                    <TableCell className="md:table-cell">
                                                    {experienceLoading&&experienceId == element._id?(
                                                    <SpecialLoadingButton></SpecialLoadingButton>
                                                      ):(
                                                    <Button className="" onClick={()=>handleExperienceDelete(element._id)}>Delete</Button>
                                                      )}
                                                    </TableCell>
                                                    <TableCell className="hidden md:table-cell">
                                                    {element.from}
